...
function printHelp {
	...
	echo " -c for client";
	echo " -s for server";
	echo "need start server first";
	echo "write two numbers (x and y) to make turn";
	echo "example: 1 3";
}
...
# для общения между сервером и клиентом используем пайп
PIPE1=/tmp/xooo-pipe1
PIPE2=/tmp/xooo-pipe2
